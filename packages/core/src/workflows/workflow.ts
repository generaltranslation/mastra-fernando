...
> {
  ...
  protected async _timeTravel<TInputSchema extends z.ZodType<any>>({
    ...
    if (this.workflowEngineType !== 'default') {
      throw new Error(`timeTravel is not supported on ${this.workflowEngineType} workflows`);
    }
    ...
    if (snapshot.status !== 'pending') {
      if (snapshot.status === 'suspended') {
        throw new Error(`This workflow run was suspended, use ${isStream ? 'resumeStream()' : 'resume()'} instead`);
      }
      throw new Error(`This workflow run is not new, current status is ${snapshot.status}`);
    }
    ...
    for (const [index, entry] of this.executionGraph.steps.entries()) {
      ...
      if (stepIds.includes(firstStep)) {
        if (index === 0 && !isNestedWorkflow) {
          throw new Error(
            `timeTravel is not needed for the first step of a workflow, use ${isStream ? 'stream()' : 'start()'} instead`,
          );
        }
        const innerExecutionPath = stepIds?.length > 1 ? [stepIds?.findIndex(s => s === firstStep)] : [];
      ...
      if (nextStepIds.length > 0 && inputData && nextStepIds.includes(firstStep)) {
        stepOutput = inputData;
      }

      stepIds.forEach(stepId => {
        stepResults[stepId] = {
          status: context?.[stepId]?.status ?? 'success',
          payload: context?.[stepId]?.payload ?? stepPayload,
          output: stepOutput ?? context?.[stepId]?.output ?? {},
          startedAt: context?.[stepId]?.startedAt ?? Date.now(),
          endedAt: context?.[stepId]?.endedAt ?? Date.now(),
          suspendedAt: context?.[stepId]?.suspendedAt,
          resumedAt: context?.[stepId]?.resumedAt,
          error: context?.[stepId]?.error ?? '',
        } as any;
      });
    }
